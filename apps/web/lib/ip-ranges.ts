 ...
 *
 * The plugin installer (and anything else making SSRF decisions) used
 * string prefixes/regexes for IPv6 — wrong for compressed notation,
...
export function isBlockedNetworkIp(ip: string): boolean {
  ...
  if (!parsed) return true; // unparseable → refuse (fail closed)
