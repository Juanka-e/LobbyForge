...
Scope boundaries: Read-only. You must not create or modify any code/config files in the repository. You can only write to your own directory `.agents/explorer_m1_1`.
Input:
...
Output requirements:
- Write an analysis report to d:\livekittest\.agents\explorer_m1_1\analysis.md with the exact recommended YAML contents for `pnpm-workspace.yaml` and package.json changes.
Completion criteria:
