...
# 21st-audit: persist the installed version — a tagged clone installs
# that tag; a plain branch clone records "source". lfctl update reads it.
GIT_VERSION="$(git -C "$SCRIPT_DIR" describe --tags --exact-match 2>/dev/null || echo source)"
...
LOBBYFORGE_VERSION=$GIT_VERSION
