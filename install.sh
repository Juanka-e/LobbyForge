...
if [ "$STACK_RUNNING" = true ]; then
  if [ -n "$EXISTING_DOMAIN" ] && [ "$EXISTING_DOMAIN" = "$DOMAIN" ]; then
    ...
    echo "  1) How to update (shows the signed-release updater commands)"
    ...
    case "$STACK_ACTION" in
      1)
        # 22nd-audit: the installer no longer performs updates. The old
        # `up -d --build` path bypassed EVERY update safety mechanism
        # (signed manifest, verified backup, digest pinning, migration
        # step, rollback state) and could silently flip a digest-pinned
        # deployment back to a mutable local build.
        echo -e "${YELLOW}Updates are handled by the signed-release updater now:${NC}"
        echo "  node scripts/lfctl.mjs update check"
        echo "  node scripts/lfctl.mjs update plan"
        echo "  node scripts/lfctl.mjs update apply --yes"
        echo ""
        echo "This installer is for fresh installs and config repair only."
...
# 21st/22nd-audit: persist the installed version — a tagged clone installs
# that tag. An untagged checkout falls back to the root package.json
# version (still valid semver for the updater — "source" would crash it).
# sed on a missing package.json exits 2 — guard the pipeline (set -e).
GIT_VERSION="$(git -C "$SCRIPT_DIR" describe --tags --exact-match 2>/dev/null || true)"
if [ -z "$GIT_VERSION" ]; then
  GIT_VERSION="$(sed -n 's/.*"version": *"\([^"]*\)".*/\1/p' "$SCRIPT_DIR/package.json" 2>/dev/null | head -1 || true)"
  if [ -n "$GIT_VERSION" ]; then
    echo -e "${YELLOW}⚠ Not a tagged checkout — recording package.json version $GIT_VERSION (source build).${NC}"
  fi
fi
if [ -z "$GIT_VERSION" ]; then
  GIT_VERSION="0.0.0-unknown"
  echo -e "${YELLOW}⚠ No tag and no package.json version found — recording $GIT_VERSION.${NC}"
fi
# 22nd-audit: a re-run on an EXISTING install must not reset the update
# model — if lfctl already moved this server to a signed digest, keep it.
DEPLOY_VERSION="$(reuse_env LOBBYFORGE_VERSION "$GIT_VERSION")"
DEPLOY_IMAGE="$(reuse_env LOBBYFORGE_IMAGE 'lobbyforge-web:latest')"
...
# Deployment state (21st-audit): lfctl reads the deployed version from
# here / deployment-state.json — never a hardcoded constant. Fresh
# installs run the locally-built image; `lfctl update apply` switches
# LOBBYFORGE_IMAGE to the digest pinned in the signed release manifest.
# Re-runs PRESERVE whatever lfctl last deployed (see DEPLOY_* above).
LOBBYFORGE_VERSION=$DEPLOY_VERSION
LOBBYFORGE_IMAGE=$DEPLOY_IMAGE
...
if [[ "$DO_CERTBOT" =~ ^[Yy]$ ]]; then
  ...
  if docker run --rm \
    ...
    certbot/certbot:v5.8.0@sha256:f70ad0adbb7e117f0fe42a63c553f28ea451edabc0148757b6efcd9735acaa20 certonly --standalone \
    -d "$DOMAIN" \
    --non-interactive \
    --agree-tos \
...
echo -e "${BOLD}To update (signed releases only — never 'git pull' on a production checkout):${NC}"
echo "  node scripts/lfctl.mjs update check"
echo "  node scripts/lfctl.mjs update plan"
echo "  node scripts/lfctl.mjs update apply --yes"
