...
- **Isolated runtime for marketplace plugins**: third-party plugin
  code now executes in a dedicated `plugin-worker` container — a
  separate process with NO host secrets, a read-only root filesystem,
  a read-only bundle mount, mem/pid caps, dropped capabilities,
  no-new-privileges, an internal-only network and a healthcheck-gated
  restart. The web app talks to it over a token-authenticated /rpc
  endpoint with hard timeouts: a hung or malicious plugin can at worst
  restart the worker, never the web app.
- **Snapshot ctx + storage capability proxy**: the worker rebuilds the
  plugin context from a SNAPSHOT envelope (actor, players, voice);
  `ctx.storage.*` executes back on the host via the internal
  `/api/internal/plugin-storage` endpoint (404 at the public nginx
  edge, constant-time token), hard-scoped to the plugin's own
  (serverId, pluginId) keyspace — the worker holds no DB credentials.
- Loader integration: dynamic execution requires BOTH the feature flag
  AND LOBBYFORGE_PLUGIN_WORKER_URL (the flag alone loads nothing —
  fail closed), and the in-process import path for third-party code is
  REMOVED from the loader. Workspace plugins (hushle, quiz) remain
  trusted in-process via the static registry.
- Compose wiring: shared plugins-data volume (web rw / worker ro),
  tokens generated by install.sh, worker built into the production
  image.
- Tests: 10 worker RPC tests (real HTTP server + fixture bundles —
  token auth, snapshot ctx, storage fail-closed on unreachable host,
  size caps) + 3 loader gate tests (flag-without-URL loads nothing,
  worker-mode loads with ZERO filesystem access, unreachable worker
  fails closed).
