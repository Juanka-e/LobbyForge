...
/**
 * Hushle's translation tables, checked against the panel that renders
...
describe('Hushle locales', () => {
  it('finds the keys the panel uses', () => {
    ...
    expect(used).toContain('hushle.lobby.packLabel');
  });
  ...
  it('ships no key the panel never renders', () => {
    ...
    expect(Object.keys(english).filter((key) => !used.includes(key))).toEqual([]);
  });
